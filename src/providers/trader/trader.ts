...
export class TraderProvider {
	...
	public quicklyBackOrder(volume, orderDirect, productId, positionId = '') {
		...
		this.present.presentConfirm(
			this.present.translateText('WAP_172','是否要执行快捷反手操作?'), function () {
				let body: any = {
					"orderFormVIce": {
						"productId": productId,
				...
				self.http.postJson('client/trade/order/get/canclose', body, function (data) {
					if (data.code == '000000') {
						...
						if (canClose == 0) {
							self.present.presentToast('WAP_129', '无可反手持仓', 'toast-red');
							return;
						}
						if (volume != canClose) {
							self.present.presentConfirm(self.present.translateText('WAP_84','可反手手数与可平手数不一致<br/>是否继续?'), function () {
				...
				function closePosition(canClose, callback = function () { }) {
					...
					self.http.postJson('client/trade/order/create', postBody, function (data) {
						if (data.code == '000000') {
							...
							if (_data.errorId == 0) {
								self.present.presentToast('WAP_287', '请求已发送', 'toast-green');
								callback();
							}
							else {
			...
			this.present.presentConfirm(self.present.translateText('', '是否要执行快捷平仓操作?'), function () {
				self.http.postJson('client/trade/order/get/canclose', body, function (data) {
					if (data.code == '000000') {
						let canClose = data.content;
						if (canClose == 0) {
							self.present.presentToast('','无可平持仓', 'toast-red');
							return;
						}
						if (volume != canClose) {
