...
export class ResignPage {
	...
	public switchName = 'ios-search';
	/* 切换手|自动支行 */
	switchBank() {
		let self = this;
		self.isInput = !this.isInput;
		setTimeout(()=>{
			self.subBankDisplayName = '';
			self.subBankChoose = '';
			if(self.isInput){
				self.switchName = 'ios-search'
			}else{
				self.switchName = 'ios-create'
			}
		});
	}
	...
	toSign() {
		if(!this.regPhone.test(this.phone) && this.config.contactChannel.phone){
	...
	presentProfileModal() {
		// 手机号|卡号回显
		this.history.cusMobile = this.phone;
		this.history.cardNo = this.cardNo;
		// 191018 选择支行 @wuwp  
		if(this.isInput){
			return;
		}
