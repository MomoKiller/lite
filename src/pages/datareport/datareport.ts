...
export class DatareportPage {
	...
	ionViewDidEnter() {
		...
		this.untraderorderRequest();
		$(document).on('blur','#datareportPartClose',() => {
			console.log('wocao');
			const val = this.trader.fixedTraderNum($('#datareportPartClose').val(), this.choosePostionInfo.minOrderVol);
			$('#datareportPartClose').val(val);
		});
	}
	/* 格式化字符串转数组 */
	...
	ionViewWillLeave() {
		...
		$(document).off('blur','#datareportPartClose');
	}
	tabMenu(num) {
		...
		const close = this.alertCtrl.create({
			...
			buttons: [
				...
				},
				{
					...
					handler: (data: any) => {
						console.log(data);
						if(Number($('#datareportPartClose').val()) > Number(this.canClosePosition)) {
						...
						const body = {
							"orderFormVIce": {
								...
								"positionId": "",
								"orderDirect": this.choosePostionInfo.orderDirect * -1,
								"orderVolume": Number($('#datareportPartClose').val()),
								"priceCondition": 1,
								"triggerPrice": 0,
								"orderPrice": 0,
								"offset": 2
